...
* Softbody: Vertices, Indices, Tetrahedra, Normals (Optional), TextureCoords(Optional)
...
|vertices|Mesh Vertices| Real list |list count must be divisible by 3|
|indices or indexes|Mesh Indices|Integer list|iteger list of indices|list count must be divisible by 3|
|normals|Mesh normals|Real list|list count must be divisible by 3|
|texturecoords|Mesh Texture coordinates|Real list|list count must be divisible by 2|
|tetrahedra|Mesh tetrahedra|Real list|list count must be divisible by 4|
...
p. END. Last modified; 09-Sept-2009, Robin Southern